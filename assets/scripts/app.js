class Product {
    constructor(title, image, desc, price ) {
        this.title = title;
        this.imageUrl = image;
        this.description = desc;
        this.price = price;
    }
}

class ShoppingCart {
    items = [];

    set cartItems(item) {
        this.items = item;
        this.totalAmount.innerHTML = `<h2>Total: \$${this.cartTotalAmount.toFixed(2)}</h2>`
    }

    get cartTotalAmount() {
        const sum = this.items.reduce((prev, curr) => prev + curr.price, 0);

        return sum;
    }

    addToCart(product) {
        const updateItems = [...this.items];
        updateItems.push(product);
        this.cartItems = updateItems
    }

    render() {
        const cartEl = document.createElement('section');
        cartEl.innerHTML = `
            <h2>Total: \$${0}</h2>
            <button>Order Now</button>
        `;
        cartEl.className = 'cart';
        this.totalAmount = cartEl.querySelector('h2')
        return cartEl
    }
}


class ProductItem {
    constructor(product) {
        this.product = product
    }
    addCartHandler() {
        App.addProductToShopping(this.product)
    }

    render() {
        const prodEl = document.createElement('li');
        prodEl.className = 'product-item';
        prodEl.innerHTML = `
            <div>
                <img src="${this.product.imageUrl}" alt"${this.product.title}"/>
                <div class="product-item__content">
                    <h2>${this.product.title}</h2>
                    <h3>\$${this.product.price}</h3>
                    <p>${this.product.description}</p>
                    <button>Add to cart</button>
                </div>
            </div>
        `;
        const addCartButton = prodEl.querySelector('button');
        addCartButton.addEventListener('click', this.addCartHandler.bind(this))
        return prodEl
    }
}

class ProductList {
    products = [
            new Product('title1', 'https://miro.medium.com/max/1200/1*mk1-6aYaf_Bes1E3Imhc0A.jpeg', 'desc1', 234.00),
            new Product('title1', 'https://miro.medium.com/max/1200/1*mk1-6aYaf_Bes1E3Imhc0A.jpeg', 'desc1', 234.00)  
    ]

    render() {
        
        const productList = document.createElement('ul');
        productList.className = 'product-list';
        for(const prod of this.products){
            const productItem = new ProductItem(prod)
            const prodEl = productItem.render()
            productList.appendChild(prodEl)
        }
        return productList
    }
}

class Shop {
    render() {
        const renderHook = document.getElementById('app');

        const product = new ProductList()
        const productList = product.render()

        this.cart = new ShoppingCart()
        const cartEl = this.cart.render()

        renderHook.appendChild(cartEl)
        renderHook.appendChild(productList)
    }
}

class App{
    static init() {
        const shop = new Shop()
        shop.render()
        this.cart = shop.cart
    }

    static addProductToShopping(product) {
        this.cart.addToCart(product)
    }
}

App.init()

